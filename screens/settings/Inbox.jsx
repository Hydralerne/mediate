...
const Inbox = ({ route, navigation }) => {
    ...
    const inbox = data?.options?.inbox
    const [sectionsState, setSectionsState] = useState(inbox);

    useEffect(() => {
        setSectionsState(inbox)
    }, [inbox])

    const handleToggle = (key) => {
        toggleCheckbox('inbox', key, setSectionsState);
    };

    const handleMenu = () => { };

    let sections = [
        'allow_only_following',
        'stop_drawing',
        'stop_music',
        'stop_movie',
        'stop_all',
                ...
                {sections?.map((section) => (
                    <TouchableButton onPress={() => handleToggle(section)} key={section}>
                        <View style={styles.section}>
                            <Text style={styles.titleText}>{t(`settings.inbox.${section}`)}</Text>
                            <CheckBox
            ...
            {/* <View style={button.buttons}>
                <TouchableButton>
                    <View style={button.section}>
                        <Image style={button.mainIcon} source={require('../../assets/icons/settings/volume mute-348-1658234823.png')} />
                        <View style={button.grid}>
                            <Text style={button.title}>Muted words</Text>
                            <Text style={button.value}>Manage your muted messages</Text>
                        </View>
